...
def main():
    net = DM2FNet_woPhy(blocks= cfgs['blocks']).cuda().train()
    ...
    while curr_iter <= cfgs['iter_num']:
        ...
        for data in train_loader:
            ...
            with amp.autocast():
                ...
                if cfgs['w_loss_l1']>0 :
                    loss_l1 = loss_x_jf  + loss_x_j1 + loss_x_j2 + loss_x_j3 + loss_x_j4
    ...
    with torch.no_grad():
        for data in tqdm(val_loader):
            ...
            for i in range(len(haze)):
                ...
                ssim = structural_similarity(g, r, data_range=1, channel_axis=-1,
                                             gaussian_weights=True, sigma=1.5, use_sample_covariance=False)
                psnr_record.update(psnr)
                ssim_record.update(ssim)

    snapshot_name = 'iter_%d_loss_%.5f_lr_%.6f_psnr_%.5f' % (curr_iter + 1, loss_record.avg, optimizer.param_groups[1]['lr'] ,psnr_record.avg)
