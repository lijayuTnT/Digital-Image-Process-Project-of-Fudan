...
class DM2FNet(Base):
    def __init__(self, blocks ,num_features=16 , conv = default_conv):
        super(DM2FNet, self).__init__()
        self.num_features = num_features
        kernel_size = 3
        
        pre_process = [conv(3, self.num_features, kernel_size)]
        self.pre = nn.Sequential(*pre_process)

        self.down1 = Group(conv, self.num_features, kernel_size, blocks=blocks)
        self.down2 = Group(conv, self.num_features, kernel_size, blocks=blocks)
        self.down3 = Group(conv, self.num_features, kernel_size, blocks=blocks)
        self.down4 = Group(conv, self.num_features, kernel_size, blocks=blocks)
...
class DM2FNet_woPhy(Base_OHAZE):
    def __init__(self, blocks ,num_features=8, conv = default_conv):
        ...
        self.down0 = Group(conv, self.num_features, kernel_size, blocks=blocks)
        self.down1 = Group(conv, self.num_features, kernel_size, blocks=blocks)
        self.down2 = Group(conv, self.num_features, kernel_size, blocks=blocks)
        self.down3 = Group(conv, self.num_features, kernel_size, blocks=blocks)
        self.down4 = Group(conv, self.num_features, kernel_size, blocks=blocks)
